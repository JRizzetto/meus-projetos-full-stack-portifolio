...
3. Neon Database + First Migration
   1. Criar projeto no Neon db: finance-tracker-dashboard
   2. Copiar a DATABASE_URL: Dashboard → Connection Details → copie a string de conexão PostgreSQL
   3. Colar no .env: Substitua DATABASE_URL="sua_url_do_neon_aqui" copiada do neon
   4. Rodar primeira migration: "npx prisma migrate dev --name init"
...
4. Prisma Client Architecture + Auth Foundation
   ...
   1. Criar arquivo de cliente Prisma
      Criar a pasta: src/lib/prisma.ts
